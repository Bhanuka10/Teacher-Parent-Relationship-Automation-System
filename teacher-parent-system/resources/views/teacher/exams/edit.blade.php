...
<style>
    ...
    .exam-subject-row {
        ...
        background: #f9fafb; border-radius: 10px; padding: 9px 13px; font-size: 13px;
    }
    .exam-subject-row + .exam-subject-row { margin-top: 6px; }
...
<div class="max-w-7xl">
    <div class="db-head">
        <div>
            <h1 class="db-title">Edit Exam</h1>
            <p class="db-sub">Update details or add extra subjects for <span>{{ $exam->name }}</span>. Existing subjects can't be renamed once marks exist.</p>
        </div>
        <div class="quick-actions">
            <a href="{{ route('teacher.exams.show', $exam) }}" class="qa-btn ghost">
    ...
    @php
        $existingSubjectsCount = $exam->subjects->count();
        $existingMarksSum = $exam->subjects->sum('max_mark');
    @endphp

    <div class="content-grid">
        <form method="POST" action="{{ route('teacher.exams.update', $exam) }}" class="space-y-5" id="exam-form">
            @csrf
            @method('PUT')
            ...
            <section class="exam-form-card space-y-4">
                <div>
                    ...
                    <input id="name" name="name" value="{{ old('name', $exam->name) }}" maxlength="255" required class="att-input">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input id="academic_year" name="academic_year" value="{{ old('academic_year', $exam->academic_year) }}" required class="att-input">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    <div>
                        ...
                        <input type="date" id="exam_date" name="exam_date" value="{{ old('exam_date', $exam->exam_date->format('Y-m-d')) }}" required class="att-input">
                    ...
                    <div>
                        ...
                        <input type="date" id="term_start_date" name="term_start_date" value="{{ old('term_start_date', $exam->term_start_date->format('Y-m-d')) }}" required class="att-input">
                    ...
                    <div>
                        ...
                        <input type="date" id="term_end_date" name="term_end_date" value="{{ old('term_end_date', $exam->term_end_date->format('Y-m-d')) }}" required class="att-input">
            ...
            <section class="exam-form-card">
                <p class="exam-field-label">Existing subjects</p>
                <div>
            ...
            <section class="exam-form-card">
                <div class="flex items-center justify-between">
                    <p class="exam-field-label" style="margin-bottom:0">Add new subjects <span class="font-normal text-gray-400" style="text-transform:none;letter-spacing:normal">(optional)</span></p>
        ...
        <div class="section-card">
            ...
            <div class="summary-body">
                <div class="summary-row"><span class="summary-label">Class</span><span class="summary-value">{{ $exam->schoolClass->name ?? '—' }}</span></div>
                ...
                <div class="summary-row"><span class="summary-label">Subjects</span><span class="summary-value" id="summary-subjects">{{ $existingSubjectsCount }}</span></div>
                <div class="summary-row"><span class="summary-label">Total Marks</span><span class="summary-value" id="summary-marks">{{ $existingMarksSum }}</span></div>
            </div>
            <div class="tips-box">
                Tip: extending dates only ever gives students more time. Existing subjects can't be renamed once marks exist for them.
            </div>
        </div>
    </div>
</div>
...
(function () {
    ...
    const baseSubjects = {{ $existingSubjectsCount }};
    const baseMarks = {{ $existingMarksSum }};
    let sCounter = 0;

    function subjectRowHtml(index) {
        return `<div class="flex items-center gap-2" data-subject-row data-sindex="${index}">
            <input type="text" name="new_subjects[${index}][name]" placeholder="Subject name" class="att-input flex-1" style="padding:8px 10px" required>
            <input type="number" name="new_subjects[${index}][max_mark]" placeholder="Max" value="100" min="1" max="1000" class="att-input" style="padding:8px 10px;width:90px" required>
    ...
    function updateSummary() {
        ...
        const newRows = subjectsContainer.querySelectorAll('[data-subject-row]');
        let newMarks = 0;
        newRows.forEach((row) => { newMarks += Number(row.querySelector('input[type="number"]')?.value || 0); });

        setText('summary-year', year || '—');
        setText('summary-term', termLabel || '—');
        setText('summary-date', dateLabel);
        setText('summary-subjects', baseSubjects + newRows.length);
        setText('summary-marks', baseMarks + newMarks);
    }
