...
@section('title', 'Staff Leave Request')

@section('content')
@php
    $statusClass = match($teacherLeaveRequest->status) {
...
@endphp
<div class="max-w-2xl">
    <a href="{{ route('admin.staff-leaves.index') }}" class="ui-back">
    ...
    <section class="ui-card mt-4 p-6">
        <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
                <span class="ui-status-pill {{ $statusClass }}">{{ ucfirst($teacherLeaveRequest->status) }}</span>
                <h1 class="mt-3 text-2xl font-bold text-gray-800">{{ $teacherLeaveRequest->teacher->name }}</h1>
                <p class="mt-1.5 text-sm text-gray-500">
                    {{ $teacherLeaveRequest->start_date->format('d M Y') }}
                    @if(!$teacherLeaveRequest->start_date->eq($teacherLeaveRequest->end_date)) – {{ $teacherLeaveRequest->end_date->format('d M Y') }} @endif
                    · {{ $teacherLeaveRequest->dayCount() }} day{{ $teacherLeaveRequest->dayCount() === 1 ? '' : 's' }}
                </p>
            </div>
        </div>

        <p class="mt-4 whitespace-pre-line border-t border-gray-100 pt-4 text-sm leading-6 text-gray-700">{{ $teacherLeaveRequest->reason }}</p>

        @if(!$teacherLeaveRequest->isPending())
            <div class="mt-4 border-t border-gray-100 pt-4 text-xs text-gray-500">
                {{ ucfirst($teacherLeaveRequest->status) }} by {{ $teacherLeaveRequest->reviewedBy?->name ?? '—' }} on {{ $teacherLeaveRequest->reviewed_at?->format('d M Y, h:i A') }}
                @if($teacherLeaveRequest->review_note)
                    <p class="mt-2 rounded-lg bg-gray-50 p-3 text-sm text-gray-700"><span class="font-semibold">Note:</span> {{ $teacherLeaveRequest->review_note }}</p>
                @endif
            </div>
        @endif
    </section>
    ...
    @if($teacherLeaveRequest->isPending())
        <section class="ui-card mt-6 p-6">
            ...
            <form method="POST" action="{{ route('admin.staff-leaves.review', $teacherLeaveRequest) }}" class="mt-4">
                @csrf
                @method('PUT')

                <label for="lr-note" class="ui-field-label">Note <span class="font-normal text-gray-400">(optional, shared with the teacher)</span></label>
