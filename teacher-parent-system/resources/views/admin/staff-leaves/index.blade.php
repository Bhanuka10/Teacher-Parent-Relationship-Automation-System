...
<div class="max-w-7xl">
    ...
    <div class="mt-header">
        <div>
            <h1 class="mt-title">Staff Leave</h1>
            <p class="mt-sub">
                Review and decide <span>leave requests</span> submitted by teachers.
            </p>
        </div>
    </div>
    ...
    <div class="mt-filter-bar">
        <form method="GET" action="{{ route('admin.staff-leaves.index') }}" id="search-form">
            <div class="mt-filter-inner">
                <div class="search-wrap">
                    ...
                    <input type="text" name="search" id="search-input"
                           value="{{ $search }}"
                           list="staff-leave-search-options"
                           placeholder="Search by teacher name or email…">
                    <datalist id="staff-leave-search-options">
                ...
                <a href="{{ route('admin.staff-leaves.index') }}" class="btn-reset-sm" id="reset-search-btn">
                    <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2">
    ...
    <div class="mt-table-wrap">
        <table id="staff-leaves-table">
            <colgroup>
                <col style="width:26%"><col style="width:18%"><col style="width:10%">
                <col style="width:28%"><col style="width:10%"><col style="width:8%">
            </colgroup>
            <thead>
                <tr>
                    <th>Teacher</th>
            ...
            <tbody>
                @forelse($leaveRequests as $leaveRequest)
                    @php
                        ...
                        $avatarColor = $colours[ord(strtolower($leaveRequest->teacher->name[0] ?? 'a')) % count($colours)];
                    @endphp
                    <tr id="staff-leave-row-{{ $leaveRequest->id }}">
                        <td>
                            <div class="user-cell">
                                <div class="user-avatar" style="background:{{ $avatarColor }}">
                                    {{ strtoupper(substr($leaveRequest->teacher->name, 0, 1)) }}
                                </div>
                                <div>
                                    <div class="user-name">{{ $leaveRequest->teacher->name }}</div>
                                    <div class="user-email">{{ $leaveRequest->teacher->email }}</div>
                                </div>
                            </div>
                        </td>
                        <td>
                        ...
                        <td>
                            <a href="{{ route('admin.staff-leaves.show', $leaveRequest) }}" class="act-btn edit" id="view-staff-leave-{{ $leaveRequest->id }}">
                                ...
                                {{ $leaveRequest->isPending() ? 'Review' : 'View' }}
                            </a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6">
                            <div class="mt-empty">
                                ...
                                <p>No staff leave requests{{ $status ? ' with this status' : '' }}{{ $search ? ' matching "'.$search.'"' : '' }}.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 18px;
            ...
            <span>Showing {{ $leaveRequests->count() }} request{{ $leaveRequests->count() === 1 ? '' : 's' }}{{ $search ? ' matching &ldquo;'.$search.'&rdquo;' : '' }}</span>
        </div>
        @endif
    </div>
